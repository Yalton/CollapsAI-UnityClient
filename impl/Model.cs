...
public abstract class Model
{
	...
	public bool CheckContradiction()
	{
		for (int x = 0; x < FMX; x++)
		{
			for (int y = 0; y < FMY; y++)
			{
				bool[] wave = this.wave[x + y * FMX];
				...
				// If no possible state found for this cell, return true indicating contradiction
				if (!found)
				{
					return true;
				}
		...
		// If no contradictions found, return false
		return false;
}
	...
	bool? Observe()
	{
		...
		// Check for contradiction after banning
		if (CheckContradiction())
		{
			throw new Exception("Contradiction found after banning.");
		}
