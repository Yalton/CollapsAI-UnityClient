...
public class SimpleTiledWFC : MonoBehaviour{
	...
	public void Run(){
		//Debug.Log("Entered Run Function");
	...
	public void Generate(){
		//Debug.Log("Entered Generate Function");
		...
		XMLName = System.IO.Path.GetFileNameWithoutExtension(xml.name);
	...
	public void Draw(){
		//Debug.Log("Entered Draw Function");
		if (output == null){
				...
				if (rendering[x,y] == null){
					...
					if (v != "?"){
						...
						if (!obmap.ContainsKey(v)){
							fab = (GameObject)Resources.Load("prefabs/" + XMLName + "/" + v, typeof(GameObject));
							if (fab == null){
								Debug.Log("Failed to load prefab: " + v);
								continue;
							}
							obmap[v] = fab;
						} else {
